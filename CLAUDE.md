...
- The project is AGPL-3.0-only. Every `.py` file starts with the two-line SPDX header (`SPDX-FileCopyrightText` + `SPDX-License-Identifier`), before the module docstring and after any shebang. New Python files must carry it.
- Config examples and templates are deliberately anonymized — no real personal locations, neighborhoods, portals, or paths into private notes. Keep example values generic.
