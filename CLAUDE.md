...
- **Real-estate** — structured listings from real-estate portals. vasco's `realestate` adapter fetches (HTTP-first, auto-escalating to Camoufox browser on bot-blocked sites) and parses each portal (vivareal/portal-a/portal-b) into normalized listing dicts; claudinho maps those to `Item`s and applies its own policy (`min_area_per_room`, `max_items`, dedup).
...
- The `youtube` config block (`ignore_shorts`, `ignore_livestreams`) resolves global→task→feed via `config.scope.layer_dict`. `ignore_shorts` is a free `/shorts/` URL check; `ignore_livestreams` fetches each new `/watch` page directly (not via vascod, which returns transcripts) to read `"isLiveContent"` — fail-open, runs last on survivors of the cheaper filters. See `pull/CLAUDE.md`.
