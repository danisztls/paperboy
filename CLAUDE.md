...
- `pull/scrapers/portal_a.py` — `PortalAAdapter`: parses listings from portal-a.com.br. Server-rendered PHP page (no JS gating, no JSON-LD); reads `.pgl-property` cards via `page.evaluate()`. Price IS on the card.
- `pull/scrapers/portal_b.py` — `PortalBAdapter`: parses listings from portal-b.com (WordPress + Elementor "Loop Grid"). Reads `.imovel.type-imovel` cards via `page.evaluate()`. Specs come from the first icon-list widget (positional: beds/baths/parking/area); location from the second. Type derived from `tipo_de_imovel-*` body classes (more stable than parsing labels). No price on the listing card.
...
- `pull/finance.py` — `FinanceSource(Source)`: yfinance quotes. yfinance is a sync library; calls run via `asyncio.to_thread`. The cfg's sub-key picks the mode:
  ...
  - **Market-hours gating**: every monitor rule is filtered through `_is_market_open(exchange, now_utc)` before the yfinance fetch. `_infer_exchange(ticker)` classifies by suffix (`.SA` → b3, `=X`/`.NYB` → fx, `X-USD` → crypto, else us_equity); per-rule `exchange:` overrides it. Schedules (`_SCHEDULES`) are stored as wall-clock times in IANA timezones — `zoneinfo` handles DST automatically. FX uses the conventional Sun 17:00 ET → Fri 17:00 ET cycle. Crypto is always open. Closed-market tickers are skipped entirely (no fetch, no alerts) but their prior state is preserved as the next open-tick baseline. Holidays are not modeled — half-day or holiday closures fetch and see no movement, which is acceptable. `_now_utc()` is a module-level indirection so tests can monkeypatch the wall clock.
