...
- `pull/scrapers/imoveis_barreto.py` — `ImoveisBarretoAdapter`: parses listings from barretoimobiliaria.com (WordPress + Elementor "Loop Grid"). Reads `.imovel.type-imovel` cards via `page.evaluate()`. Specs come from the first icon-list widget (positional: beds/baths/parking/area); location from the second. Type derived from `tipo_de_imovel-*` body classes (more stable than parsing labels). No price on the listing card.
