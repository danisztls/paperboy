...
- `DiscordEmbedTarget` — each item as a Discord embed. The embed image is `Item.image` (set during pull from the feed entry or during summarize from the article's `og:image`); `item.meta["skip_image"]` (from task/feed `image.skip`) suppresses it. When `Item.images` has more than one URL, the target sends up to 4 embeds sharing `Item.url` — Discord merges them into one visual embed with a gallery layout (title/description/footer live on the first embed only). Falls back to a single-image embed when `Item.url` is missing or only one image is available.
