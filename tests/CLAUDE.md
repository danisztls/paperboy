...
Tests call `_process_feed_task` and `_process_research_task` directly (not the `_async_main` orchestrator, not the CLI). Two boundaries are faked; everything else is real.

- **`FakeLLMAdapter`** (`conftest.py`) — `LLMAdapter` subclass with two queues: `queue()` / `queue_text()` for `complete()` calls and `queue_structured()` / `queue_filter()` for `complete_structured()` calls. Passed as `curate_adapter=` and `summarize_adapter=` on `_process_feed_task`, or `research_adapter=` on `_process_research_task` — no monkeypatching needed. Exhausted queues raise loudly so missing canned responses fail the test rather than hanging. The research loop additionally fakes `process._vasco.search`/`extract` (see `test_research.py`'s `FakeVasco`/`patch_vasco`).
