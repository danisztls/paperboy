...
- `feed_state.py` — `merge_feed_state` (per-feed item merge: stamp `first_seen`, carry summary/filter annotations, drop failed posts) and `build_feed_task_state` (all feeds + the memory log: capped at `MEMORY_MAX_ENTRIES`=20, last `MEMORY_CONTEXT_ENTRIES`=5 sent to the LLM).
