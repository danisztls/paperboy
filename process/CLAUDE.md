...
- Filter criteria come from `curate_cfg["criteria"]`; the coverage ledger (passed as `ledger=`) is rendered into the prompt by `_format_ledger` and drives Step 2 dedup + the explicit-`freq` escalating-trajectory bar.
